...
class SystemController {
    ...
    async getSystemSettings(req, res) {
        try {
            ...
            return (0, helper_1.sendResponse)(res, 200, "System settings retrieved successfully", settings);
        }
        catch (error) {
    ...
    async updateSystemSettings(req, res) {
        try {
            ...
            if (!settings || typeof settings !== "object") {
                return (0, helper_1.sendResponse)(res, 400, "Valid settings object is required");
            }
            // Validate settings structure
            const validSections = ["business", "inventory", "sales", "notifications"];
            const invalidSections = Object.keys(settings).filter((key) => !validSections.includes(key));
            if (invalidSections.length > 0) {
                return (0, helper_1.sendResponse)(res, 400, `Invalid settings sections: ${invalidSections.join(", ")}`);
            }
            ...
            return (0, helper_1.sendResponse)(res, 200, "System settings updated successfully", {
    ...
    async getSystemStatus(req, res) {
        try {
            const [userCount, productCount, saleCount, databaseStatus, recentErrors] = await Promise.all([
            ...
            return (0, helper_1.sendResponse)(res, 200, "System status retrieved successfully", systemStatus);
        }
        catch (error) {
     ...
     */
    async backupSystemData(req, res) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return (0, helper_1.sendResponse)(res, 400, "Authentication required");
            ...
            return (0, helper_1.sendResponse)(res, 200, "System backup created successfully", backupInfo);
        }
        catch (error) {
