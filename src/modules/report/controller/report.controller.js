...
class ReportController {
    ...
    async generateSalesReport(req, res) {
        try {
            const { startDate, endDate, groupBy = "daily", // daily, weekly, monthly
            reportType = "summary", // summary, detailed
             } = req.query;
            if (!startDate || !endDate) {
                return (0, helper_1.sendResponse)(res, 400, "Start date and end date are required");
            }
            ...
            const [salesData, productData, paymentData] = await Promise.all([
                this.getSalesReportData(dateFilter, groupBy),
            ...
            return (0, helper_1.sendResponse)(res, 200, "Sales report generated successfully", reportData);
        }
        catch (error) {
    ...
    async getSalesReportData(dateFilter, groupBy) {
        let groupByField;
        switch (groupBy) {
            case "daily":
    ...
    async getProductSalesData(dateFilter) {
        const productSales = await prisma_1.default.saleItem.groupBy({
            by: ["productId"],
            where: {
                sale: dateFilter,
            },
    ...
    async getPaymentMethodData(dateFilter) {
        return await prisma_1.default.sale.groupBy({
            by: ["paymentMethod"],
            where: dateFilter,
            _sum: {
                finalAmount: true,
    ...
    async generateInventoryReport(req, res) {
        try {
            ...
            const [stockLevels, lowStockItems, stockMovements, inventoryValue] = await Promise.all([
                this.getStockLevels(),
            ...
            return (0, helper_1.sendResponse)(res, 200, "Inventory report generated successfully", reportData);
        }
        catch (error) {
    ...
    async getLowStockItems() {
        return await prisma_1.default.$queryRawUnsafe(`SELECT * FROM "Product" WHERE "isActive" = true AND "quantity" <= "minStock" ORDER BY "quantity" ASC`);
    }
    async getStockMovements() {
        return await prisma_1.default.inventoryLog.findMany({
            take: 50,
    ...
    async getInventoryValue() {
        const result = await prisma_1.default.product.aggregate({
            where: { isActive: true },
            _sum: {
                quantity: true,
            },
